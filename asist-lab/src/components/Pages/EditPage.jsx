...
const EditPage = ({ setIsAuthenticated }) => {
  const location = useLocation();
  const { tripToEdit } = location.state || {};
  ...
  function formatToDateTime(inputDate) {
  ...
  useEffect(() => {
    if (tripToEdit) {
      setTitle(tripToEdit.name);
      setDescription(tripToEdit.description);
      setStartDate(formatToDateTime(tripToEdit.expectedStartTime));
      setEndDate(formatToDateTime(tripToEdit.expectedFinishTime));
      setRoutePoints(
        tripToEdit.points.map(point => ({ lat: point.latitude, lng: point.longitude }))
      );
    }
  }, [tripToEdit]);
    ...
    <div>
      ...
      <div className="flex flex-col md:flex-row p-8 max-w-5xl mx-auto">
        <form onSubmit={handleSubmit} className="md:w-1/3 md:mr-4">
          <h1 className="text-3xl mb-6">Edit Trip</h1>
        ...
        <div className="mt-6 md:mt-0 md:w-2/3">
          <h2 className="text-lg mb-2">Edit Route:</h2>
          <MapContainer
            center={[52.4, 31]}
            zoom={10}
